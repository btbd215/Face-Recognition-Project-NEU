...
#add_data(root= r"C:\Users\KyThuat88\Face-Recognition-Project\add_data")


class FaceRecognition:
    def get_train_test_splits(self, folder, test_image_path=None, train_size=0.8, random_state=42):
        ...
        if test_image_path:
            ...
            test_pixels = np.asarray(test_image).reshape(1, -1)
    ...
    def __init__(self, folder=r'dataset',test_image_path=None, train_size=0.8, random_state=42):
        X_train, X_test, y_train, y_test = self.get_train_test_splits(
            folder=folder, train_size=train_size, random_state=random_state,test_image_path=test_image_path)
    ...
    def incremental_pca(self, n_components = 150,batch_size=None, verbose = False):
        ...
        self.z = feature_weights
        if verbose:
            print(f'After extraction: Train shape {self.X_train.shape}, Test shape: {self.X_test.shape}')
            print('Total variance explained: ', np.sum(
                pca.explained_variance_ratio_))
    ...
    def feature_extraction(self, n_components=150, verbose=False, pca_method='pca',tsne_params={'n_components': 2, 'perplexity': 30},**kwargs):
        ...
        if pca_method == 'pca':
            ...
            self.modular_pca(n_components=40, verbose=False)
            return
        ...
        feature_weights = np.sum(
            (self.pca.components_ ** 2) * explained_variances.reshape(-1, 1), axis=0)
        self.z = feature_weights
        if verbose:
            print('Total variance explained: ', np.sum(
                self.pca.explained_variance_ratio_))

    def plot_eigen(self,n_components = 150):
        eigenface = self.eigenfaces.reshape((n_components,112,92))
        n_rows = 2
        n_cols = 5
    ...
    def model_selection(self, model='Random Forest Optuna', param=None, verbose=True, n_trials=100, distance='euclidean', version='normal', plot=False, method='both',num_prediction = 5,plot_eigen = False):
        with timer(model):
            ...
            if 'Optuna' in model_name:
                ...
                if plot:
                    ...
                    self.plot_ml_based(method = method,num_prediction=num_prediction)
                gc.collect()
            ...
            if model_name == 'Distance Based':
                ...
                if plot:
                    self.plot_distance_based(
                        distance=distance, distances=distances, pairs=pairs, y_pred=y_pred, method=method,num_prediction=num_prediction)
            else:
    ...
    def plot_distance_based(self, distance='euclidean', distances=None, pairs=None, y_pred=None, method='both',num_prediction = 5):
        if method == 'both':
            self.visualize_wrong_and_correct_predictions(
                distance, distances, pairs, y_pred, num_predictions=num_prediction)
        elif method == 'wrong':
    ...
    def visualize_wrong_and_correct_predictions(self, distance, distances, pairs, y_pred,num_predictions = 5):
        ...
        true_predictions = [(i, pairs[i][1]) for i in range(
            len(pairs)) if self.y_test[i] == y_pred[i]]
        self.visualize_correct_top_predictions(
                distance, distances, pairs, y_pred,num_predictions=num_predictions)
        self._visualize_predictions(
            wrong_predictions, 'wrong', f'Wrong Predictions - {distance}')

        
    def visualize_correct_top_predictions(self, distance, distances, pairs, y_pred, num_predictions=10):
        true_predictions = [(i, pairs[i][1]) for i in range(len(pairs)) if self.y_test[i] == y_pred[i]]

        if len(true_predictions) > 0:
            # Sort true predictions based on distance
            sorted_predictions = sorted(true_predictions, key=lambda x: distances[x[0]])

            # Get top num_predictions predictions
            top_predictions = sorted_predictions[:num_predictions]
            #top_predictions = true_predictions[:num_predictions]

            # for i in range(num_predictions):
            #     print(
            #         f'Top matching pairs for {distance} (Top {i+1} Smallest Distance: {distances[i]}):')
            print(f'Top matching pairs for {distance}')
            for i, (index, _) in enumerate(top_predictions):
                print(
                f'Top {i+1} Smallest Distance: {distances[index]}')

            # Visualize the top predictions
            #self._visualize_predictions(top_predictions, "True Positive", f'top_true_positive_{distance}')
        else:
    ...
    def _visualize_predictions(self, predictions, folder_name, title):
        if len(predictions) > 0:
            ...
            for i, (test_index, train_index) in enumerate(predictions, start=1):
                ...
                #test_image_path = r'.\add_data\s2\3.pgm'
                ...
                plt.title(f'Test Image (Actual: {test_folder}, Predicted: {self.y_train[train_index]})',fontsize = 2)
                plt.axis('off')

                plt.subplot(len(predictions), 2, i * 2)
                plt.imshow(train_image, cmap='gray')
                plt.title(f'Predicted Train Image (Folder: {train_folder})',fontsize = 2)
                plt.axis('off')
    ...
    def plot_ml_based(self,method = "both",num_prediction = 5):
        y_predict = self.predicted_class
        if method == 'both':
            self.plot_wrong_and_correct_ml(num_prediction = num_prediction)
        elif method == "correct":
            self.plot_images_ml(np.where(self.y_test == y_predict), num_prediction = num_prediction)
        else:
            self.plot_images_ml(np.where(self.y_test != y_predict))
    
    def plot_wrong_and_correct_ml(self, num_prediction = None):
        y_predict = self.predicted_class
        correct_indices = np.where(self.y_test == y_predict)
        if num_prediction != None:
            correct_indices = correct_indices[:num_prediction]
        misclassified_indices = np.where(self.y_test != y_predict)
        self.plot_images_ml(correct_indices)
        self.plot_images_ml(misclassified_indices)

    def plot_images_ml(self, classified_indices, num_prediction = None):
        y_predict = self.predicted_class
        plt.figure(figsize=(15, 5 * len(classified_indices[0])))

        for i, idx in enumerate(classified_indices[0], start=1):
            if num_prediction is not None and i > num_prediction:
                break
            test_folder = f"s{self.y_test[idx]}"
            predicted_class = f"s{y_predict[idx]}"

            # Load the test image
            test_image_path = os.path.join('dataset', test_folder, f"{idx % 10 + 1}.pgm")
            test_image = Image.open(test_image_path)
            ...
            plt.subplot(len(classified_indices[0]), 2, i * 2 - 1)
            plt.imshow(test_image, cmap='gray')
            plt.title(f'Test Image (Actual: {test_folder}, Predicted: {predicted_class})',fontsize = 10)
            plt.axis('off')

            plt.subplot(len(classified_indices[0]), 2, i * 2)
            plt.imshow(predicted_image, cmap='gray')
            plt.title(f'First Image of Predicted Class ({predicted_class})',fontsize = 10)
            plt.axis('off')

        plt.tight_layout()
        #plt.savefig(f'predicted class.png')
        plt.show()
...
def main(model, folder=r'.\dataset', train_size=0.8, param=None,
         verbose=True, n_components=150, n_trials=20, distance='euclidean', version='normal', plot=False, method='both', num_prediction = 5, random_state=42, pca_method='pca',test_image_path = None,plot_eigen = False):
    '''
    ...
    with timer('Model Selection'):
        fdm.model_selection(model=model, param=param,
                            verbose=verbose, n_trials=n_trials, distance=distance, version=version, plot=plot, method=method, num_prediction = num_prediction,plot_eigen = plot_eigen)
        gc.collect()
...
if __name__ == '__main__':
    ...
    #          param=best_params[i], n_components=dataset, random_state=0, verbose=False)
    main(model = 'Distance Based',folder=r'.\dataset', train_size=0.8,distance='mahala',n_components=150, random_state=42, verbose=False, plot=True,method='correct', pca_method="pca",num_prediction=5,test_image_path = None,plot_eigen=False)
    #main(model='SVM', folder=r'.\dataset', train_size=0.8, param={'C': 68.62612780012446, 'gamma': 0.0028340054896037214, 'kernel': 'linear'}, pca_method="ica",n_components=95, plot = False,method = "wrong",num_prediction=5,test_image_path=None,plot_eigen=False)
    ...
    #main(model='MLP Optuna', folder=r'.\dataset', train_size=0.8,n_components=50, random_state=42, verbose=False, plot=False, n_trials=50)
